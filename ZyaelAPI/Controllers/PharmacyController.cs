...
namespace ZyaelAPI.Controllers
{
    ...
    public class PharmacyController : ControllerBase
    {
        ...
        public async Task<IActionResult> PharmacyVendorCredentialAdd(int PharmacyVendorID)
        {
            PharmacyModel item = new PharmacyModel();
            if (PharmacyVendorID > 0)
            {
                item = await _pharmacy.PharmacyVendorCredentialAdd(PharmacyVendorID);

                item.PharmacyVendorID = PharmacyVendorID;
            }
        ...
        public async Task<IActionResult> PharmacyVendorCredentialDetailsDelete(int PharmacyVendorID)
        {
            //HospitalModel result = new HospitalModel();

            var respose = await _pharmacy.PharmacyVendorCredentialDetailsDelete(PharmacyVendorID);
            if (Response != null)
            {
